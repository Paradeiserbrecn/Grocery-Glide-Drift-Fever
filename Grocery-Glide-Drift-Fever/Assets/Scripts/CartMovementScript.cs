...
public class CartMovement : MonoBehaviour
{
	...
	private void FixedUpdate()
	{
		if (!ragdoll)
		{
			...
			_tippingThreshold += (fixedTippingThreshold - _tippingThreshold) * _tippingDecaySpeed;
			_driftBoost += (maxBoostStrength - _driftBoost) * _boostDecaySpeed;

			HandleBoostParticles();

			//Debug.Log(_cart.velocity.magnitude + ", " +_cart.angularVelocity.magnitude);
			return; // You should not perform any other action in FixedUpate if ragdoll is false
		}
	...
	private void HandleInputAxisRaw()
	{
		...
		switch (_verticalAxis)
		{
			//thrust
			case > 0:
				_cart.AddForce(transform.forward * (_verticalAxis * thrust * _driftBoost * 110f));
				break;
			//brake
			case < 0:
				_cart.AddForce(transform.forward * (_verticalAxis * thrust * _driftBoost * 40f));
				break;
		}

		if (_horizontalAxis != 0) //rotation
		{
			_cart.AddTorque(transform.up * (_horizontalAxis * angular * 50f));
		}
	}
