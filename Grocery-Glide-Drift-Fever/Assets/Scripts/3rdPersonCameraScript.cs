...
public class ThirdPersonCamera : MonoBehaviour
{
    ...
    private RaycastHit _ragdollHitInfo;
    ...
    private void Update()
    {
        if (!cart.ragdoll)
        {
            ...
            bool raycastHit = Physics.Raycast(_lookPoint, _direction, out _hitInfo, _direction.magnitude,
                LayerMask.GetMask("Environment"));

            if (raycastHit)
            {
                _globalCameraPos = _hitInfo.point + _hitInfo.normal * 0.2f;
            }

            transform.position = Vector3.SmoothDamp(transform.position, _globalCameraPos, ref _vel, smooth);
            transform.forward = (_cartTransform.position - (transform.position + _cartTransform.up * lookHeight))
                .normalized;
        }
        else
        {
            _globalCameraPos = _cartTransform.position +
                               (transform.position - _cartTransform.position).normalized * ragdollDist;
            _direction = _globalCameraPos - _cartTransform.position;
            _lookPoint = _cartTransform.position - _cartTransform.up * lookHeight;

            Debug.DrawLine(_lookPoint, _globalCameraPos, Color.red);
            bool deiMama = Physics.Raycast(_lookPoint, _direction, out _ragdollHitInfo, _direction.magnitude,
                LayerMask.GetMask("Environment"));
            if (deiMama)
            {
                _globalCameraPos = _ragdollHitInfo.point + _ragdollHitInfo.normal * 0.2f;
            }
            ...
            transform.forward = Vector3.SmoothDamp(transform.forward,
                (_cartTransform.position - (transform.position)).normalized, ref _ragdollDirVel, smooth);
        }
    }
}